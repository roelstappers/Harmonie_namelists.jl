#  We support two patterns for strings in the YAML files
#  1) a string starting with $ is taken from the  enviroment and errors if the ENV variable is not set
#  2) a string starting with $ and containing a ? will use the ENV variable or the default value after the ? 
